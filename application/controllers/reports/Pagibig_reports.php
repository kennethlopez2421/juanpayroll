...
class Pagibig_reports extends CI_Controller {
  public function __construct(){
    parent::__construct();
    $this->load->model('reports/pagibig_reports_model');
    $this->isLoggedIn();
  }
  ...
  public function get_pagibig_reports_json(){
    $search = json_decode($this->input->post('searchValue'));
    $data = $this->pagibig_reports_model->get_pagibig_reports_json($search);
  ...
  public function index($token = ""){
    ...
    $this->load->view('reports/pagibig_reports',$data);
  }

  public function check_export_to_excel_pagibig_reports(){
    $this->isLoggedIn();
    $search = json_decode($this->input->post('search'));
    $pagibig_reports_count = $this->pagibig_reports_model->check_export_to_excel_pagibig_reports($search)->num_rows();

    if($pagibig_reports_count > 0){
  ...
  public function export_to_excel_pagibig_reports($month = ""){
    ...
    $title = $month." Pagibig Reports";
    $header = $month." Pagibig Reports";
    $pagibig_reports_data = $this->pagibig_reports_model->export_to_excel_pagibig_reports($month);
    $pagibig_reports_count = $this->pagibig_reports_model->export_to_excel_pagibig_reports($month)->num_rows();
    $last_index = $pagibig_reports_count + 3;
    ...
    $x= 2;
    $j = 1;
    foreach($pagibig_reports_data->result_array() as $sub){
        $spreadsheet->setActiveSheetIndex(0)
            ->setCellValue("A$x",$sub['pagibig_no'])
            ...
            ->setCellValue("H$x",$sub['company_name'])
            ->setCellValue("I$x",$sub['total']);
        $x++;
        $j++;
    }
