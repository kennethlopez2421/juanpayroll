...
class Attendance_graph_analysis extends CI_Controller {
  public function __construct(){
    parent::__construct();
    $this->load->model('reports/attendance_graph_analysis_model');
    $this->isLoggedIn();
  }
  ...
  public function get_graph_analysis_json(){
    date_default_timezone_set('Asia/Manila');
    $filter_by = $this->input->post('filter_by');
    $emp_idno = $this->input->post('emp_idno');
    $fullname = $this->model->get_emp_fullname($emp_idno)->row()->fullname;
    $worksched = $this->input->post('worksched');
    ...
    switch ($filter_by) {
      case 'this_month':
            ...
            for($i = 0; $i < 7; $i++){
              if($day == $days[$i]){
                if($worksched[$days[$i]][0] != ""){
                  $timelog = $this->attendance_graph_analysis_model->get_timelog($emp_idno,$date);
          ...
          (
            ...
            "fullname" => '<strong><u>'.$fullname.'</u></strong>'
          );
        break;
      case 'last_3months':
  ...
  public function index($token = ""){
    ...
    $this->load->view('reports/attendance_graph_analysis',$data);
  }
}
