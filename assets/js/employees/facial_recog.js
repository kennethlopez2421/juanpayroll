$(function(){
  ...
  var facial_recog = function (){
    ...
    var isloggein = $('#isloggein').val();
    if(isloggein == false){
      // SUBMIT ACCOUNT CODE FORM
      $(document).on('submit', '#account_code_form', function(e){
        e.preventDefault();
        var error = 0;
        var errorMsg = "";
        ...
        if(error == 0){
          $.ajax({
            url: base_url+'employees/Timelog/facial_recog_index',
            type: 'post',
            ...
            success: function(data){
              $.LoadingOverlay('hide');
              if(data.success == 1){
                window.location.href = base_url+'employees/Timelog/facial_recog_index'
              }else{
      ...
      $('#account_code_modal').modal({ backdrop: 'static', keyboard: false });
      return ;
    }
  ...
  function check_sess_id(admin) {
    ...
    $.ajax({
      ...
      error: function(){
        $.LoadingOverlay('hide');
        notificationError('Connection not available', 'Unable to connect to the internet. Please try again.');
        processing = false;
        limit = 36;
      }
    });
  ...
  function refresh_time(){
    $.ajax({
      url: base_url+'employees/Timelog/refresh_time',
      type: 'post',
      success: function(data){
      }
    });
  ...
  setInterval(() => {refresh_time()},2700000);
