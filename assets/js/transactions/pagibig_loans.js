$(function(){
  ...
  function gen_pagibig_loan_tbl(search){
    var pagibig_loan_tbl = $('#pagibig_loan_tbl').DataTable( {
      "processing": true,
      "serverSide": true,
      "searching": false,
      "destroy": true,
      ...
      "ajax":{
        url: base_url+'transactions/Pagibig_loans/get_pagibig_loans_json',
        type: 'post',
  ...
  function gen_view_pagibig_loan_tbl(search){
    var view_pagibig_loan_tbl = $('#view_pagibig_loan_tbl').DataTable( {
      "processing": true,
      "serverSide": false,
      "searching": false,
      "destroy": true,
      ...
      "ajax":{
        url: base_url+'transactions/Pagibig_loans/get_pagibig_loan',
        type: 'post',
        data: { searchValue: search },
        beforeSend: function(){
          $.LoadingOverlay('show');
  ...
  gen_pagibig_loan_tbl(JSON.stringify(searchValue));
  ...
  // SUBMIT PAGIBIG LOAN FORM
  $(document).on('submit', '#pagibig_loan_form', function(e){
    e.preventDefault();
    var error = 0;
    var errorMsg = "";
    ...
    var total_loan = $('#total_loan').data('raw');
    var monthly_amortization = $('#monthly_amortization').data('raw');
    ...
    if(error == 0){
      $.ajax({
        url: base_url+'transactions/Pagibig_loans/create',
        type: 'post',
        contentType: false,
        processData: false,
        data: formData,
  ...
  $(document).on('keyup', '#employee', function(){
    ...
    if(employee != ""){
      $.ajax({
        url: base_url+'transactions/Pagibig_loans/search_user_w_sss',
        type: 'post',
        data:{ employee },
        beforeSend: function(){
          // $.LoadingOverlay('show');
  ...
  $(document).on('click', '.btn_view_modal', function(){
    var pagibig_loan_id = $(this).data('pagibig_loan_id');
    var pagibig_total_loan = $(this).data('pagibig_total_loan');
    var monthly_amortization = $(this).data('monthly_amortization');

    $('#view_total_loan').val(pagibig_total_loan);
    $('#view_monthly_amortization').val(monthly_amortization);
    gen_view_pagibig_loan_tbl(pagibig_loan_id);
    $('#view_modal').modal();
  })
