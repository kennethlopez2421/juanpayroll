	...
	$(".searchBtn").click(function(e){
		...
		if(checker == 1)
		{
			var dataTable = $('#table-grid').DataTable({
				...
				"ajax":{
					url:base_url+"Main_inventory/inventory_table", // json datasource
	...
	$('#table-grid').delegate(".btnView", "click", function(){
	  	...
	  	$.ajax({
	  		...
	  		success:function(data){
	  			...
	  			$('#inventory_name').val(data.itemname);
				$('#id').val(data.id);
				$('#inventory_category').val(data.catid);
				$('#unit').val(data.uomid);
				if(data.isforsale=="1"){$('#is_for_sale').prop("checked", true);}
				if(data.webforsale=="1"){$('#web_for_sale').prop("checked", true);}
				if(data.trackinventorycount=="1"){$('#track_inventory_count').prop("checked", true);}
				if(data.isportal=="1"){$('#is_portal_for_sale').prop("checked", true);}
				if(data.isarchive=="1"){$('#is_archive').prop("checked", true);}
