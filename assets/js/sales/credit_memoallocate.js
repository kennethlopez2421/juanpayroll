$(function(){
	...
	var dataTable = $('#table-grid').DataTable({
		...
		"destroy":true,
		"ajax":{
			url :base_url+"Main_sales/table_creditcollection_allocate", // json datasource
	...
	$("#form1").submit(function(e){
        ...
        serializedForm.push({
         ...
         $.ajax({
                type:'post',
                url:base_url+'Main_sales/tbl_creditcollectionallocate_save',
                data: serializedForm,
                beforeSend: function() {
                    $.LoadingOverlay("show");
                },
                ...
                if(data.success == 1)
                    ...
                    window.setTimeout(function(){
                                 window.location.href=base_url+"Main_sales/credit_memosummary/" + token;
                          }, 500);    
                        $.toast({
                            heading: 'Success',
                            text: 'Credit Memo Allocation has been successfully updated.',
                            icon: 'success',
                            loader: false,  
                            stack: false,
                            position: 'top-center', 
...
function amountAllocateCollection(count,unpaidamount)
{
    ...
    var fieldrun = 'dr';
    var drpaybalance = document.getElementById("drpaybalance").value;
