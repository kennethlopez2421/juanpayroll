$(function(){
  ...
  function gen_pagibig_reports_tbl(filter){
    var pagibig_reports_tbl = $('#pagibig_reports_tbl').DataTable( {
      "processing": true,
      "pageLength": 10,
      "serverSide": true,
      "searching": false,
      ...
      "ajax":{
        url: base_url+'reports/Pagibig_reports/get_pagibig_reports_json',
        type: 'post',
        data: {
          searchValue: filter
        },
  ...
  gen_pagibig_reports_tbl(JSON.stringify(filter));
  ...
  $(document).on('click', '#btnExport', function(){
    ...
    $.ajax({
      url: base_url+'reports/Pagibig_reports/check_export_to_excel_pagibig_reports',
      type: 'post',
      ...
      success: function(data){
        $.LoadingOverlay('hide');
        if(data.success == 1){
          window.open(base_url + 'reports/Pagibig_reports/export_to_excel_pagibig_reports/'+month);
        }else{
